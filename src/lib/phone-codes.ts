// Popular codes shown first, then the full list of countries alphabetically.
...
export function findPhoneCountryByCode(code: string): PhoneCountryCode | undefined {
  return PHONE_COUNTRY_CODES.find((entry) => entry.code === code);
}

export function formatPhoneCountryOption(entry: PhoneCountryCode) {
  return `${entry.code} ${entry.label}`;
}

export function parseStoredPhone(phone?: string, phoneCountryCode?: string) {
  const fallbackCode = phoneCountryCode || "+91";
  if (!phone) return { countryCode: fallbackCode, number: "" };

  const sorted = [...PHONE_COUNTRY_CODES].sort((a, b) => b.code.length - a.code.length);
  const match = sorted.find((entry) => phone.startsWith(entry.code));
  if (match) {
    return {
      countryCode: match.code,
      number: phone.slice(match.code.length).replace(/\D/g, ""),
    };
  }
...
export function formatStoredPhone(countryCode: string, number: string) {
